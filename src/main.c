...
double gt=0;
double sine_amp=23000, f=0.2, period;
int recv_fail_cnt=0;
...
boolean ecat_init(void)
{
	...
	{
		rt_printf("1. ec_init on %s succeeded.\n", ecat_ifname); //ifname
		/* find and auto-config slaves */

		if ( ec_config_init(FALSE) > 0 )
		{
			rt_printf("2. %d slaves found and configured.\n",ec_slavecount);
			...
			for (i=0; i<NUMOFMAXPOS_DRIVE; ++i)
			{
				...
				printf("Set the CSP mode using SDO.\n");
			}
#endif
			...
			ec_configdc();
#endif
			rt_printf("3. Slaves mapped, state to SAFE_OP.\n");
			rt_printf("-----------------------------------\n");
